...
async def collect_statistics(
    ...
    while not stop_event.is_set():
        # -- Port-level totals (used for TX pkts, RX pkts, and fps) --
        p1_tx_tot, p1_rx_tot, p2_tx_tot, p2_rx_tot = await asyncio.gather(
        ...
        # P1->P2: stream1 has tpld_id=0, so port2 receives on tpld(0)
        # P2->P1: stream2 has tpld_id=1, so port1 receives on tpld(1)
        p2rx_lat, p2rx_jit, p2rx_err, p1rx_lat, p1rx_jit, p1rx_err = await utils.apply(
        ...
        # -- per-second rates --
        tx_fps    = p1_tx_tot.packet_count_last_sec   # P1 TX rate
        p2_rx_fps = p2_rx_tot.packet_count_last_sec   # P2 RX rate (P1->P2)
        p2_tx_fps = p2_tx_tot.packet_count_last_sec   # P2 TX rate
        p1_rx_fps = p1_rx_tot.packet_count_last_sec   # P1 RX rate (P2->P1)

        logging.info(f"[{frame_size}B] s={count+1:3d}  "
                     f"P1->P2 TX={tx_fps}pps RX={p2_rx_fps}pps  "
                     f"lat_avg={p2rx_lat.avg_val}ns  loss={p2rx_err.non_incre_seq_event_count}  |  "
                     f"P2->P1 TX={p2_tx_fps}pps RX={p1_rx_fps}pps  "
                     f"lat_avg={p1rx_lat.avg_val}ns  loss={p1rx_err.non_incre_seq_event_count}")

        # P1->P2 direction: P1 is TX, P2 is RX
        per_second_p1.append({
            "frame_size":   frame_size,
            "TX_fps":       tx_fps,
            "RX_fps":       p2_rx_fps,
            "TX_pkts":      p1_tx_tot.packet_count_since_cleared,
            "RX_pkts":      p2_rx_tot.packet_count_since_cleared,
        ...
        # P2->P1 direction: P2 is TX, P1 is RX
        per_second_p2.append({
            "frame_size":   frame_size,
            "TX_fps":       p2_tx_fps,
            "RX_fps":       p1_rx_fps,
            "TX_pkts":      p2_tx_tot.packet_count_since_cleared,
            "RX_pkts":      p1_rx_tot.packet_count_since_cleared,
