...
def _parse_args():
    ...
    # Positional args
    if len(args) < 4:
        print("Usage: xena_rfc2544.py <duration> <frame_size> <port1> <port2> [--rfc2544] [--loss-threshold <float>]")
        sys.exit(1)
    ...
    return duration, frame_size, port1, port2, run_rfc2544, loss_threshold


TRAFFIC_DURATION, FRAME_SIZE, PORT1, PORT2, RUN_RFC2544, LOSS_THRESHOLD = _parse_args()
...
async def _set_rate(stream1, stream2, rate_fraction: int) -> None:
...
async def _get_latency(
    ...
    """Return average latency stats from both ports (ns)."""
...
async def rfc2544_throughput(
    ...
    for iteration in range(cfg.throughput_max_iterations):
        ...
        p1_tx, p2_rx, p2_tx, p1_rx = await _get_frame_counts(port1, port2)
        loss_fwd = _loss_pct(p1_tx, p2_rx)
        loss_rev = _loss_pct(p2_tx, p1_rx)
        worst_loss = max(loss_fwd, loss_rev)
...
async def rfc2544_latency(
    ...
    for i in range(cfg.latency_iterations):
        ...
        await _traffic_on(port1, port2)
        await asyncio.sleep(cfg.trial_duration)
        lat = await _get_latency(port1, port2)
        await _traffic_off(port1, port2)
        await asyncio.sleep(1)
        samples.append(lat)
        logging.info(
...
async def rfc2544_back_to_back(
    ...
    await _set_rate(stream1, stream2, 1_000_000)  # 100% line rate
    ...
    while burst_sec <= cfg.b2b_max_sec:
        ...
        loss_fwd = _loss_pct(p1_tx, p2_rx)
        loss_rev = _loss_pct(p2_tx, p1_rx)
        no_loss = (loss_fwd == 0.0 and loss_rev == 0.0)
...
async def _create_and_configure_streams(port_obj_1, port_obj_2, frame_size: int):
    """Create streams on both ports and return (stream1, stream2)."""
    logging.info(f"Creating streams for frame_size={frame_size}B")
    ...
    try:
        for frame_size in RFC2544_FRAME_SIZES:
            ...
            stream1, stream2 = await _create_and_configure_streams(
                port_obj_1, port_obj_2, frame_size
            )
            ...
            tp_result = await rfc2544_throughput(
                port_obj_1, port_obj_2, stream1, stream2, frame_size, cfg
            )
            frame_results["throughput"] = tp_result
            throughput_pct = tp_result["throughput_pct"]

            # 2. Latency (at throughput rate)
            lat_result = await rfc2544_latency(
                port_obj_1, port_obj_2, stream1, stream2, frame_size, throughput_pct, cfg
            )
