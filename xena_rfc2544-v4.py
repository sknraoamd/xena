...
async def run_rfc2544(
    ...
    logging.info("=" * 70)
    logging.info(f"{'Size':>6}  {'Throughput':>12}  {'AvgLat P2-RX':>14}  {'MaxB2B':>10}  {'Note':>14}")
    logging.info(f"{'(B)':>6}  {'(% line)':>12}  {'(ns)':>14}  {'(sec)':>10}  {'':>14}")
    logging.info("-" * 70)
    for r in all_results:
        sz  = r["frame_size_bytes"]
        tp  = r["throughput"]["throughput_pct"]

        # Latency - may be skipped or have None avg_ns
        lat_val = r.get("latency", {}).get("p2_rx", {}).get("avg_ns")
        lat_str = f"{lat_val:>14.0f}" if lat_val is not None else f"{'N/A':>14}"

        # Back-to-back - may be skipped
        b2b_val = r.get("back_to_back", {}).get("max_burst_no_loss_sec")
        b2b_str = f"{b2b_val:>10.2f}" if b2b_val is not None else f"{'N/A':>10}"

        note = "INDETERMINATE" if r["throughput"].get("indeterminate") else ""
        logging.info(f"{sz:>6}  {tp:>12.3f}  {lat_str}  {b2b_str}  {note:>14}")
